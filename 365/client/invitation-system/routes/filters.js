 ...
 */
  ...
  //will fix names once it's fully funcitonal
  Router.onBeforeAction(checkUserLoggedIn, {
    except: ['index', 'signup2', 'signup2/:token', 'login2', 'recover-password', 'reset-password']
  });

  Router.onBeforeAction(userAuthenticatedBetaTester, {
    only: ['index', 'signup2', 'signup2/:token', 'login2', 'recover-password', 'reset-password', 'invites']
  });

  Router.onBeforeAction(userAuthenticatedAdmin, {
    only: ['index', 'signup2', 'signup2/:token', 'login2', 'recover-password', 'reset-password']
  });
