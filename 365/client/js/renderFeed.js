...
Template.main.onRendered(function() {
  ...
  renderFeed('#myFeed', 'myFeed-container', {userId:Meteor.userId(), createdAt: {$gte:start}});
  renderFeed('#worldFeed', 'worldFeed-container', {userId:{$ne: Meteor.userId()}, createdAt: {$gte:start}});
    renderFeed('#friendFeed','friendFeed-container',{friendList:Meteor.userId(), createdAt: {$gte:start}});
});
...
function setupStage(containerDiv, canvas) {
  ...
  if (containerDiv === '#myFeed') {
    ...
    worldStage = stage;
    return worldStage;
  } else if (containerDiv == '#friendFeed'){
      friendStage = stage;
      return friendStage;
  } else {
    return stage;
